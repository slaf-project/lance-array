...
**Not** a full Zarr implementation: **2D only**. Reads decode every Lance tile that intersects the selected indices (fancy reads may widen the window). Writes merge updated tiles in one batch per assignment; design notes live under [`prds/`](https://github.com/slaf-project/lance-array/tree/main/prds).
